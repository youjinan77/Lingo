(() => {
  ...
  words.forEach(w => {
    const meaning = {pos:w.pos,meaning:w.meaning,example:exampleMap.get(w.word+'|'+w.pos)||additions[w.word]||''};
  ...
  function pool(){return $('deck').value==='day12'?day12:$('deck').value==='weak'?weak:$('deck').value==='unlearned'?all.filter(w=>!known.includes(w)):all;}
  function start(){
  ...
  $('ws').oninput=rw;rw();
  const daily=$('daily');
  if(daily){daily.querySelector('h2').textContent='Day 12 学习已完成';const summary=daily.querySelector('summary');if(summary)summary.textContent='Day 12 完成内容';
